...
class SubPostController extends AdminController
{
    ...
    public function index($typePost)
    {
        ...
        return View('admin.sub_post.list', compact( 'typePost', 'typeSubPost'));
    }
    ...
    public function edit($typePost, SubPost $subPost )
    {
        ...
        return view('admin.sub_post.edit', compact(
            'templates',
            'typeInputs', 'post',
            'typePost', 'typeSubPost', 'subPost', 'languages',
            'posts',
            'typeInputsGeneral'
        ));
    }
    ...
    public function anyDatatables(Request $request, $typePost) {
        ...
        $data = Datatables::of($posts)
            ->addColumn('index', function ($post) {
                 if ($post->index_hot == 1) {
                     return '<label><input type="checkbox" onclick="return changeIndexHot(this)" postId="'.$post->post_id.'" class="flat-red" value="1" checked /> nổi bật</label>';
                 }

                 return '<label><input type="checkbox" onclick="return changeIndexHot(this)" postId="'.$post->post_id.'" class="flat-red" value="0" /> nổi bật</label>';
            })
